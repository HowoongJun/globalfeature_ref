...
def vgg16NetvladPca(image_batch):
    ...
    with tf.variable_scope('vgg16_netvlad_pca'):
        ...
        if image_batch.shape[3] == 1:
            ...
            # x = tf.keras.layers.Conv2D(image_batch, np.ones((1, 1, 1, 3)), 
            #                       np.ones(4).tolist(), 'VALID')
        else :
        ...
        def vggConv(inputs, numbers, out_dim, with_relu):
            if with_relu:
                ...
                activation = None
            return tf.layers.Conv2D(out_dim, [3, 3], 1, padding='same',
                                    activation=activation, 
                                    name='conv%s' % numbers)(inputs)
        def vggPool(inputs):
            return tf.layers.MaxPooling2D(2, 2)(inputs)
        ...
        # PCA
        x = tf.layers.Conv2D(4096, 1, 1, name='WPCA')(tf.expand_dims(tf.expand_dims(x, 1), 1))
        x = tf.nn.l2_normalize(tf.layers.Flatten()(x), axis=-1, name = "descriptor")
